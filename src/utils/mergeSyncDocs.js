...
// with the activity log — the sets/reps plans, the custom routines, and today's
// focuses. It has no imports so the exact same code runs in the browser bundle
// and in the Node sync server, guaranteeing both sides resolve conflicts
// identically.
//
...
export const SYNC_DOC_NAMES = ["plans", "customWorkouts", "dailyFocus"];
...
// Merge the whole `{ plans, customWorkouts, dailyFocus }` bag, keeping only the documents
