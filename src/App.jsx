...
function CheckCard({ label, detail, reps, done, color, onToggle, tip }) {
  return (
    <div style={{ background:T.surface,border:`1px solid ${T.border}`,borderRadius:14,marginBottom:8,overflow:"hidden" }}>
      <div onClick={onToggle} style={{ display:"flex",gap:14,alignItems:"flex-start",padding:"14px 16px",cursor:"pointer" }}>
        ...
        <div style={{ flex:1 }}>
          ...
          {detail && <p style={{ fontSize:12,color:done?T.dim:T.muted,lineHeight:1.5 }}>{detail}</p>}
          {tip && !done && <div style={{ marginTop:8,background:`${color}10`,borderLeft:`3px solid ${color}`,borderRadius:6,padding:"8px 10px",fontSize:11,color }}>💡 {tip}</div>}
        </div>
      </div>
    </div>
  );
...
const foamTech = [
  ...
  { area:"Hip Flexors", emoji:"🟠", detail:"Face down, roller angled under hip/groin. Keep toes on floor, roll gently across hip flexor area.", tip:"Move slowly. Hold on tight spots for 20 sec." },
  { area:"Thoracic Spine", emoji:"🟢", detail:"Roller horizontal across mid-back. Hands behind head, knees bent. Let upper back extend over roller. Scoot up slightly for each vertebra.", tip:"Never roll on the neck or lower lumbar spine." },
  { area:"Plantar Fascia", emoji:"🟡", detail:"Standing or seated, roller (or ball) under foot arch. Roll from heel to ball of foot slowly.", tip:"Use a lacrosse ball for deeper, more targeted work." },
];
  ...
  return (
    <div style={{ minHeight:"100vh",background:T.bg,fontFamily:font,color:T.text,paddingBottom:60 }}>
      ...
      <div style={{ maxWidth:500,margin:"0 auto",padding:"0 20px" }}>
        ...
        {foamTech.map((t,i) => (
          <CheckCard key={i} label={`${t.emoji} ${t.area}`} detail={t.detail} color={color} tip={t.tip}
            done={!!checked[`foam-${i}`]} onToggle={() => setChecked(p => ({ ...p, [`foam-${i}`]:!p[`foam-${i}`] }))}/>
        ))}
...
const stretches = [
  { name:"Neck Rolls", hold:"5 reps/side", detail:"Slowly roll head in a half-circle, ear to shoulder to chest." },
  { name:"Chest Opener", hold:"30 sec", detail:"Clasp hands behind back, lift arms, open chest and look up." },
  { name:"Hip Flexor Stretch", hold:"30 sec/side", detail:"Kneeling lunge, push hips forward. Feel the stretch in the front hip." },
  { name:"Hamstring Stretch", hold:"30 sec/side", detail:"Seated or standing, reach toward toes with a flat back." },
  { name:"Quad Stretch", hold:"30 sec/side", detail:"Standing, pull foot to glute. Keep knees together, stand tall." },
  { name:"Shoulder Cross-Body", hold:"20 sec/side", detail:"Pull one arm across chest. Apply gentle pressure above elbow." },
  { name:"Spinal Twist", hold:"30 sec/side", detail:"Seated, cross one foot over opposite knee, rotate torso toward raised knee." },
  { name:"Child's Pose", hold:"45 sec", detail:"Kneel, sit back on heels, arms extended forward. Breathe deeply." },
  { name:"Pigeon Pose", hold:"45 sec/side", detail:"Front leg bent at 90°, back leg extended. Deep hip opener." },
  { name:"Calf Stretch", hold:"30 sec/side", detail:"Hands on wall, back leg straight, heel flat on floor. Lean in gently." },
];

function StretchScreen({ onBack, checked, setChecked }) {
  const color = T.green;
  const done = stretches.filter((_,i) => checked[`str-${i}`]).length;
  return (
    <div style={{ minHeight:"100vh",background:T.bg,fontFamily:font,color:T.text,paddingBottom:60 }}>
      <ScreenHeader title="Stretch" subtitle="FLEXIBILITY · 10 MIN" emoji="🧘" color={color} onBack={onBack}/>
      <div style={{ maxWidth:500,margin:"0 auto",padding:"0 20px" }}>
        <ProgressBar done={done} total={stretches.length} color={color}/>
        {stretches.map((s,i) => (
          <CheckCard key={i} label={s.name} detail={s.detail} reps={s.hold} color={color}
            done={!!checked[`str-${i}`]} onToggle={() => setChecked(p => ({ ...p, [`str-${i}`]:!p[`str-${i}`] }))}/>
        ))}
        {done === stretches.length && <CompletionBanner color={color} emoji="🌿" text="FULLY STRETCHED!"/>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight:"100vh",background:T.bg,fontFamily:font,color:T.text,paddingBottom:60 }}>
      ...
      <div style={{ maxWidth:500,margin:"0 auto",padding:"0 20px" }}>
        ...
        {simpleEx.map((s,i) => (
          <CheckCard key={i} label={s.name} detail={s.detail} reps={s.reps} color={color}
            done={!!checked[`sim-${i}`]} onToggle={() => setChecked(p => ({ ...p, [`sim-${i}`]:!p[`sim-${i}`] }))}/>
        ))}
    ...
    <div style={{ minHeight:"100vh",background:T.bg,fontFamily:font,color:T.text,paddingBottom:60 }}>
      ...
      <div style={{ padding:"16px 20px 0",maxWidth:500,margin:"0 auto" }}>
        ...
        {w.steps.map((step,si) => {
          ...
          return (
            <div key={si} style={{ background:ts.bg,borderRadius:14,marginBottom:8,overflow:"hidden" }}>
              <div onClick={() => setChecked(p => ({ ...p,[key]:!p[key] }))}
                style={{ display:"flex",gap:14,alignItems:"flex-start",padding:"14px 16px",cursor:"pointer" }}>
                <div style={{ width:24,height:24,borderRadius:7,border:`2px solid ${ac}`,background:done?ac:"transparent",flexShrink:0,marginTop:2,display:"flex",alignItems:"center",justifyContent:"center" }}>
                  {done && <span style={{ fontSize:13,color:"#000",fontWeight:800 }}>✓</span>}
                </div>
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4 }}>
                    <span style={{ fontWeight:700,fontSize:14,color:done?T.muted:T.text,textDecoration:done?"line-through":"none" }}>{step.phase}</span>
                    <span style={{ fontSize:10,fontWeight:700,color:ac,background:`${ac}18`,borderRadius:99,padding:"3px 9px" }}>{step.reps}</span>
                  </div>
                  <p style={{ fontSize:12,color:done?T.dim:T.muted,lineHeight:1.5 }}>{step.detail}</p>
                </div>
              </div>
            </div>
          );
