...
- Trigger point reference/checklist by body area, opened by an interactive **trigger-point hotspot map**. The final front/back anatomy artwork and dedicated plantar inset carry 54 tappable hotspots across the 19 regions; tapping one asks "Focus on <muscle> today?" and, on yes, adds that specific hotspot (region + side) to today's focuses, where it shows on Home alongside stretch and simple-exercise focuses. Measured source-pixel centres live in `src/data/triggerPointHotspots.js` and are converted to panel percentages at render time, so they hold at any screen size. Two baked anterior-deltoid dots are intentionally display-only because anterior deltoid is not among the 19 curated regions. The dots mark commonly described regions, not exact anatomical coordinates.
...
- **Cross-device sync (optional):** your data is local by default, but you can connect a device to a shared **sync code** (Activity Log → “Sync across devices”). Enter the same code on your phone and desktop and four things stay in sync through the server API (`/api/sync/:code`): the activity log, the sets/reps plans, your custom routines, and today's focuses. There are no accounts — the code is the shared key, so pick something only you would guess (the server stores only its SHA-256 hash). The log merges entry by entry (newest edit wins) with deletions propagating via tombstones; plans, custom routines and today's focuses are whole documents, so the most recently edited copy wins. Nothing is silently overwritten or resurrected. See `src/hooks/useCloudSync.js`, `src/hooks/useSyncedDoc.js`, `src/utils/mergeActivityLog.js`, `src/utils/mergeSyncDocs.js`, `server/store.js`, and `server/index.js`.
- **The weekly plan is published, read-only, at `GET /api/weekly-plan`.** The fixed Mon–Sun rota in `src/data/weeklyPlan.js` is this app's to decide, and the Main Hub's Daily Dashboard shows today's row of it under its Countdowns card. Rather than let the hub keep a second copy that drifts, the route hands back `{ date, today, week }` — `today` being the plan for `?date=YYYY-MM-DD` (the caller's own calendar day; this server's day when the parameter is left off) and each item carrying a `path` such as `/#/workout-sets` so a caller can link straight into the screen. No token, and an `Access-Control-Allow-Origin: *` header, because a rota is not personal data: no log, no sync code, nothing about a person ever travels on it. See `server/index.js` and `src/utils/weeklyPlanDay.js`, which is also what the Weekly Plan screen uses to put today on top.
- **Today's focuses are published, read-only, at `GET /api/daily-focus`.** Whatever you tapped as a focus today — a stretch, a simple exercise, a trigger point — is already in the sync record, so the Main Hub's Daily Dashboard reads it back from there and shows it as a pill at the top of the morning page rather than keeping a list of its own. The route hands back `{ date, focuses, updatedAt }`, each focus carrying a `path` such as `/#/stretch` or `/#/simple/wall-sit` so it can be tapped straight through to where it came from. Unlike the weekly plan this is personal, so it answers only to the sync code — in an `x-sync-code` header, because a code in a query string ends up in an access log — and carries no CORS header. A list from another day is not today's answer, so it reads as no focuses; a code that has never synced one answers 404, which is a setup step rather than an outage. See `server/index.js` and `src/utils/dailyFocus.js`.
